...
const fold = (paper: number[][], instructions: string[], printResult: boolean = false): number => {
  instructions.forEach(inst => {
    ...
    if (dir === 'y') {
      paper.slice(+cut - 1).reverse().forEach((line, x) => line.forEach((v, y) => paper[x][y] += v));
      paper = paper.slice(0, +cut - 1);
    } else if (dir === 'x') {
      paper.forEach((line, y) => {
        line.slice(+cut).reverse().forEach((v, x) => line[x] += v);
        paper[y] = line.slice(0, +cut);
      })
    }
  });
  printResult && visualize(paper);
  return paper.flat().filter(v => v > 0).length;
}
